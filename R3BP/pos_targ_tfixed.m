...
function soln = pos_targeter_tfixed(mu, targ, z_ic, tf, ode_opts, max_iter, tol)
    ...
    while 1
        ...
        [T{i},Z{i}] = ode113(@stm_phidot_A_phi,[0,tf],Z0{i},ode_opts,mu);
        phi_tf{i} = reshape(Z{i}(end,7:end),[6 6]);

        % Compute the error -- err = r_target-r_err
        err(:,i) = targ-Z{i}(end,1:3)';
        norm(err);
        ...
        phi_rv{i} = phi_tf{i}(1:3,4:6); % Top right block of the STM matrix
        dv(:,i) = phi_rv{i}\err(:,i);

        % Compute the new initial conditions - v_i+1 = v_i + dv_i
        z_ic(:,i+1) = z_ic(:,i) + [0;0;0;dv(:,i)]; % Updated initial state
